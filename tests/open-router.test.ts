...
describe('open-router adapter manifest', () => {
  ...
  it('covers the full activepieces action set (models.list, completions.create)', () => {
    const names = openRouterConnector.manifest.capabilities.map((c) => c.name).sort()
    expect(names).toEqual(['completions.create', 'models.list'].sort())
    ...
    const mutations = openRouterConnector.manifest.capabilities
      ...
      .map((c) => c.name)
      .sort()
    expect(reads).toEqual(['models.list'].sort())
    expect(mutations).toEqual(['completions.create'].sort())
  })
})
