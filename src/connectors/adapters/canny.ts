...
export const cannyConnector = declarativeRestConnector({
  ...
  capabilities: [
    ...
    },
    {
      name: 'posts.update',
      class: 'mutation',
      description: 'Update fields on an existing Canny post.',
      parameters: {
        type: 'object',
        properties: {
          postID: { type: 'string', description: 'Canny post id to update.' },
          title: { type: 'string' },
          details: { type: 'string' },
          eta: { type: 'string', description: 'Estimated delivery date in MM/YYYY.' },
          etaPublic: { type: 'boolean' },
    ...
    },
    {
      name: 'posts.delete',
      class: 'mutation',
      description: 'Delete a Canny post by id.',
      parameters: {
        type: 'object',
        properties: { postID: { type: 'string', description: 'Canny post id to delete.' } },
        required: ['postID'],
      },
      request: {
        method: 'POST',
    ...
    },
    {
      ...
      class: 'mutation',
      description: 'Add a comment to a Canny post on behalf of a user.',
      parameters: {
        type: 'object',
        properties: {
          postID: { type: 'string', description: 'Post being commented on.' },
          authorID: { type: 'string', description: 'Canny user ID writing the comment.' },
          value: { type: 'string', description: 'Comment body (plain text).' },
          parentID: { type: 'string', description: 'Optional parent comment id to thread under.' },
          imageURLs: { type: 'array', items: { type: 'string' }, description: 'Image attachment URLs.' },
          internal: { type: 'boolean', description: 'Whether the comment is internal-only.' },
          createdAt: { type: 'string', description: 'ISO-8601 created-at for migrated comments.' },
        },
        required: ['postID', 'authorID', 'value'],
      },
      request: {
        method: 'POST',
