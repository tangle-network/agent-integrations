...
export const trelloConnector = declarativeRestConnector({
  ...
  capabilities: [
    ...
    },
    {
      name: 'lists.archive',
      class: 'mutation',
      description: 'Archive a list (closed=true) without deleting it.',
      parameters: {
        type: 'object',
        properties: {
          key: { type: 'string' },
      ...
      request: {
        method: 'PUT',
        path: '/1/lists/{listId}/closed',
        query: { key: '{key}', value: 'true' },
      },
      cas: 'native-idempotency',
      externalEffect: true,
    },
    {
      name: 'boards.update',
      class: 'mutation',
      description: 'Update a board (rename, description, prefs, closed state).',
      parameters: {
        type: 'object',
        properties: {
          key: { type: 'string' },
          ...
          'prefs/permissionLevel': { type: 'string', enum: ['private', 'org', 'public'] },
    ...
    {
      name: 'checklists.checkItem',
      class: 'mutation',
      description: 'Mark a check item complete (or incomplete) on a card.',
      parameters: {
        type: 'object',
        properties: {
          key: { type: 'string' },
          ...
          state: { type: 'string', enum: ['complete', 'incomplete'], description: 'Defaults to complete.' },
        },
        required: ['key', 'cardId', 'checkItemId'],
      },
      request: {
        method: 'PUT',
        path: '/1/cards/{cardId}/checkItem/{checkItemId}',
        query: { key: '{key}', state: '{state}' },
      },
      cas: 'native-idempotency',
      externalEffect: true,
    },
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          color: {
            type: 'string',
            description: 'Trello label color (yellow, purple, blue, red, green, orange, black, sky, pink, lime, null).',
          },
          idBoard: { type: 'string' },
        },
        required: ['key', 'name', 'color', 'idBoard'],
      },
      request: {
        method: 'POST',
        path: '/1/labels',
    ...
    },
    {
      ...
      class: 'mutation',
      description: 'Create a webhook subscribing to events on a Trello model (board/card/list/member).',
      parameters: {
        type: 'object',
        properties: {
          key: { type: 'string' },
          callbackURL: { type: 'string', description: 'Public HTTPS endpoint Trello will POST events to.' },
          idModel: { type: 'string', description: 'Trello object id (board, card, list, member) to subscribe to.' },
          description: { type: 'string' },
          active: { type: 'boolean' },
        },
        required: ['key', 'callbackURL', 'idModel'],
      },
      request: {
        method: 'POST',
        path: '/1/webhooks',
