...
const MEMBER_SCOPES = ['openid', 'profile', 'email', 'w_member_social'] as const
...
const ORGANIZATION_SCOPE_SET = new Set<string>(ORGANIZATION_SCOPES)

export interface LinkedInOptions {
  /** Enable LinkedIn Marketing Developer Platform organization capabilities. */
...
export function createLinkedinConnector(
  ...
  const organizationAccess = options.organizationAccess === true
  const postScope = organizationAccess
    ? 'w_organization_social'
    : 'w_member_social'
  ...
  return declarativeRestConnector({
    ...
    description:
      organizationAccess
        ? 'Read the connected LinkedIn member profile and administered organizations, and create / read / delete organization posts and comments via the LinkedIn REST API.'
        : 'Read the connected LinkedIn member profile and create / delete member posts and comments via the LinkedIn REST API.',
    auth: {
      kind: 'oauth2',
      authorizationUrl: 'https://www.linkedin.com/oauth/v2/authorization',
      tokenUrl: 'https://www.linkedin.com/oauth/v2/accessToken',
      scopes: organizationAccess
    ...
    capabilities: ([
      ...
      },
      {
        ...
        description:
          organizationAccess
            ? 'Create a LinkedIn organization post on the REST Posts API. `author` is an organization URN (e.g. `urn:li:organization:{id}`). `commentary` is the post text; `distribution.feedDistribution` defaults to `MAIN_FEED`; `lifecycleState` defaults to `PUBLISHED`. Append-only — caller owns dedupe via idempotencyKey.'
            : 'Create a LinkedIn member post on the REST Posts API. `author` is the connected member URN (e.g. `urn:li:person:{sub}`). `commentary` is the post text; `distribution.feedDistribution` defaults to `MAIN_FEED`; `lifecycleState` defaults to `PUBLISHED`. Append-only — caller owns dedupe via idempotencyKey.',
        parameters: {
          type: 'object',
          properties: {
            author: {
              type: 'string',
              description: organizationAccess
                ? 'URN of the organization post author (urn:li:organization:{id}).'
                : 'URN of the connected member post author (urn:li:person:{sub}).',
            },
      ...
      },
      {
        ...
        description:
          organizationAccess
            ? 'Create a top-level comment on an organization share / post. `actor` is the organization URN posting the comment; `message.text` is the comment text. Append-only — caller owns dedupe via idempotencyKey.'
            : 'Create a top-level comment on a share / post as the connected member. `actor` is the member URN posting the comment; `message.text` is the comment text. Append-only — caller owns dedupe via idempotencyKey.',
        parameters: {
          type: 'object',
          properties: {
            shareUrn: {
            ...
            actor: {
              type: 'string',
              description: organizationAccess
                ? 'URN of the commenting organization.'
                : 'URN of the commenting member.',
            },
            message: {
              type: 'object',
    ...
    ] satisfies RestOperationSpec[]).filter(
      // Organization-gated capabilities are not part of the self-serve grant.
      // Keep them absent from the default manifest so discovery cannot advertise them.
      ({ requiredScopes }) =>
        organizationAccess ||
