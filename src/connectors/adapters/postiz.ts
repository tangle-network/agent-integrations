  ...
  capabilities: [
    ...
    },
    {
      ...
      parameters: {
        type: 'object',
        properties: {
          postId: { type: 'string', description: 'The ID of the post to update' },
          content: { type: 'string', description: 'Updated text content' },
          type: { type: 'string', enum: ['now', 'draft', 'schedule'], description: 'Post type' },
          date: { type: 'string', description: 'ISO 8601 publish date for scheduled posts' },
          media: { type: 'object', description: 'Media file paths to attach' },
          shortLink: { type: 'boolean', description: 'Automatically shorten URLs' },
          settings: { type: 'object', description: 'Platform-specific settings' },
      ...
      request: { method: 'PUT', path: '/posts/{postId}', body: 'args' },
      cas: 'native-idempotency',
      externalEffect: true,
    },
    {
      name: 'posts.schedule',
      class: 'mutation',
      description: 'Schedule a post for a future time. Pass type="schedule" plus the ISO date.',
      parameters: {
        type: 'object',
        properties: {
          content: { type: 'string', description: 'Text content of the post' },
          type: { type: 'string', enum: ['schedule'], description: 'Must be "schedule".' },
          date: { type: 'string', description: 'ISO 8601 publish date' },
          media: { type: 'object', description: 'Media file paths to attach' },
          shortLink: { type: 'boolean', description: 'Automatically shorten URLs' },
          settings: { type: 'object', description: 'Platform-specific settings' },
        },
        required: ['content', 'type', 'date'],
      },
      request: {
        method: 'POST',
        path: '/posts',
    ...
    },
    {
      ...
      parameters: {
        type: 'object',
        properties: {
          integrationId: { type: 'string', description: 'The ID of the integration to disconnect' },
        },
        required: ['integrationId'],
      },
      request: { method: 'DELETE', path: '/integrations/{integrationId}' },
      cas: 'native-idempotency',
      externalEffect: true,
    },
    {
      ...
      parameters: {
        type: 'object',
        properties: {
          mediaId: { type: 'string', description: 'The ID of the media asset to delete' },
        },
        required: ['mediaId'],
      },
      request: { method: 'DELETE', path: '/media/{mediaId}' },
      cas: 'native-idempotency',
      externalEffect: true,
    },
    {
