...
export const senjaConnector = declarativeRestConnector({
  ...
  capabilities: [
    ...
    },
    {
      ...
      class: 'mutation',
      description: 'Edit fields on an existing testimonial. Only provided fields are updated.',
      parameters: {
        type: 'object',
        properties: {
          id: { type: 'string', description: 'The unique ID of the testimonial to update.' },
          title: { type: 'string', description: 'A short headline for the testimonial.' },
          text: { type: 'string', description: 'The body of the testimonial.' },
          rating: { type: 'integer', description: 'Star rating from 1 to 5.' },
          approved: { type: 'boolean', description: 'Mark as approved.' },
          customer_name: { type: 'string', description: 'Full name of the person giving the testimonial.' },
          customer_email: { type: 'string', description: 'Email address of the customer.' },
          customer_company: { type: 'string', description: 'Company or organisation the customer belongs to.' },
          customer_tagline: { type: 'string', description: 'Short role or tagline for the customer.' },
      ...
      request: { method: 'PATCH', path: '/testimonials/{id}', body: 'args' },
      cas: 'native-idempotency',
      externalEffect: true,
    },
    {
      ...
      parameters: {
        type: 'object',
        properties: {
          id: { type: 'string', description: 'The unique ID of the testimonial to delete.' },
        },
        required: ['id'],
      },
      request: { method: 'DELETE', path: '/testimonials/{id}' },
      cas: 'native-idempotency',
      externalEffect: true,
    },
    {
      name: 'tags.create',
      class: 'mutation',
      description: 'Create a tag that can be used to organize testimonials.',
      parameters: {
        type: 'object',
        properties: {
          name: { type: 'string', description: 'Name of the tag.' },
          color: { type: 'string', description: 'Optional color for the tag (hex or named).' },
        },
        required: ['name'],
      },
      request: { method: 'POST', path: '/tags', body: 'args' },
      cas: 'native-idempotency',
      externalEffect: true,
    },
    {
      ...
      class: 'read',
      description: 'List testimonial collections (widgets) on the workspace.',
      parameters: {
        type: 'object',
        properties: {
