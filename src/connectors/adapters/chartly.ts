...
export const chartlyConnector = declarativeRestConnector({
  ...
  capabilities: [
    ...
    },
    {
      name: 'update.chart',
      class: 'mutation',
      description: 'Update an existing chart with new data, labels, or styling.',
      parameters: {
        type: 'object',
        properties: {
          chart_id: { type: 'string', description: 'Identifier of the chart to update.' },
          chart_type: { type: 'string' },
          chart_title: { type: 'string' },
          labels: { type: 'array', items: { type: 'string' } },
          dataset_label: { type: 'string' },
          data_values: { type: 'array', items: { type: 'number' } },
          background_color: { type: 'string' },
          width: { type: 'integer', minimum: 1, maximum: 2000 },
          height: { type: 'integer', minimum: 1, maximum: 2000 },
          format: { type: 'string', enum: ['png', 'svg'] },
          background_color_image: { type: 'string' },
          advanced_config: { type: 'object' },
        },
        required: ['chart_id'],
      },
      request: {
    ...
    },
    {
      name: 'delete.chart',
      class: 'mutation',
      description: 'Delete a chart by identifier.',
      parameters: {
        type: 'object',
        properties: {
          chart_id: { type: 'string', description: 'Identifier of the chart to delete.' },
        },
        required: ['chart_id'],
      },
      request: {
    ...
    },
    {
      name: 'share.chart',
      class: 'mutation',
      description: 'Share a chart with a recipient via email or generate a shareable link.',
      parameters: {
        type: 'object',
        properties: {
          chart_id: { type: 'string', description: 'Identifier of the chart to share.' },
          recipient: {
            type: 'string',
            description: 'Email address to share with. Omit to generate a public link instead.',
          },
          permission: {
            type: 'string',
            enum: ['view', 'edit'],
            description: 'Recipient permission level. Defaults to view.',
          },
          message: { type: 'string', description: 'Optional message attached to the share invite.' },
        },
        required: ['chart_id'],
      },
      request: {
        method: 'POST',
        path: '/charts/{chart_id}/share',
        body: {
          recipient: '{recipient}',
