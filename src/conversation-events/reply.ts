...
export function buildMessagingReply(
  ...
  if (event.provider === 'inkbox') {
    if (event.eventType === 'inkbox.imessage.received') {
      return { ok: true, reply: { action: 'inkbox.imessage.reply', input: { conversation_id: event.conversationId, text } } }
    }
    if (event.eventType === 'inkbox.text.received') {
      ...
      return { ok: true, reply: { action: 'inkbox.sms.reply', input: { phone_number_id: message.phone_number_id, conversation_id: event.conversationId, text } } }
    }
    if (event.eventType === 'inkbox.message.received') {
      ...
      return { ok: true, reply: { action: 'inkbox.email.send', input: {
  ...
  if (event.provider === 'contiguity') {
    return { ok: true, reply: { action: event.eventType === 'contiguity.imessage.incoming' ? 'contiguity.messages.send_imessage' : 'contiguity.sms.send',
