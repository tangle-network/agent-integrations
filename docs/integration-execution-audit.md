...
| Native adapter backends | 449 |
| Native adapter surfaces shipped | 548 |
| Package-runtime backends | 220 |
| Runtime manifest dependencies for catalog-only connectors | 221 |
| Catalog-only connectors exposable behind runtime | 220 |
| Catalog-only actions exposable behind runtime | 950 |
...
...and 468 more native adapter surfaces.
...
| Native adapter execution | Done for listed native backends | 548 reviewed native adapter surfaces ship from this package; 449 overlap the 669 catalog contracts. |
| OAuth/API-key setup metadata | Partial | 184 setup specs exist; 150 are executable setup specs and 34 are catalog/setup-only. |
| Direct adapter backlog | Tracked | 220 contracts still need native/direct adapters before they should be product-executable. |
...
| --- | ---: | --- |
| Contracts needing native/direct adapters | 220 | Connector has a Tangle contract but no reviewed direct adapter yet. |
...
1. **Tangle first-class connector contracts are complete.**
   All 669 catalog entries have Tangle-owned contracts. 449 use native adapter backends; 220 are backlog for native ports.
...
5. **Native adapter coverage is intentionally smaller than contract breadth.**
   This repo ships 548 native adapter surfaces. 449 overlap the 669 catalog contracts; the remaining catalog contracts are not product-executable until ported.
...
- It is accurate to say: **the remaining 220 catalog-only contracts are backlog, not runtime-ready product surface.**
