...
| Native adapter backends | 496 |
| Native adapter surfaces shipped | 595 |
| Package-runtime backends | 173 |
| Runtime manifest dependencies for catalog-only connectors | 174 |
| Catalog-only connectors exposable behind runtime | 173 |
| Catalog-only actions exposable behind runtime | 793 |
...
...and 515 more native adapter surfaces.
...
| Native adapter execution | Done for listed native backends | 595 reviewed native adapter surfaces ship from this package; 496 overlap the 669 catalog contracts. |
| OAuth/API-key setup metadata | Partial | 231 setup specs exist; 199 are executable setup specs and 32 are catalog/setup-only. |
| Direct adapter backlog | Tracked | 173 contracts still need native/direct adapters before they should be product-executable. |
...
| --- | ---: | --- |
| Contracts needing native/direct adapters | 173 | Connector has a Tangle contract but no reviewed direct adapter yet. |
...
1. **Tangle first-class connector contracts are complete.**
   All 669 catalog entries have Tangle-owned contracts. 496 use native adapter backends; 173 are backlog for native ports.
...
5. **Native adapter coverage is intentionally smaller than contract breadth.**
   This repo ships 595 native adapter surfaces. 496 overlap the 669 catalog contracts; the remaining catalog contracts are not product-executable until ported.
...
- It is accurate to say: **the remaining 173 catalog-only contracts are backlog, not runtime-ready product surface.**
