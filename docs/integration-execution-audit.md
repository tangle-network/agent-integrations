...
| Native adapter execution | Done for listed native backends | 621 reviewed native adapter surfaces ship from this package; 516 overlap the 669 catalog contracts. |
| OAuth/API-key setup metadata | Partial | 246 setup specs exist; 226 are executable setup specs and 20 are catalog/setup-only. |
...
5. **Native adapter coverage is intentionally smaller than contract breadth.**
   This repo ships 621 native adapter surfaces. 516 overlap the 669 catalog contracts; the remaining catalog contracts are not product-executable until ported.
