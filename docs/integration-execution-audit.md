...
| Setup specs | 246 |
| Executable setup specs | 233 |
| Catalog/setup-only specs | 13 |
...
| Native adapter surfaces shipped | 626 |
...
...and 546 more native adapter surfaces.
...
| Native adapter execution | Done for listed native backends | 626 reviewed native adapter surfaces ship from this package; 516 overlap the 669 catalog contracts. |
| OAuth/API-key setup metadata | Partial | 246 setup specs exist; 233 are executable setup specs and 13 are catalog/setup-only. |
...
| Commercial/setup-only provider contracts | 13 | Provider is discoverable with honest setup metadata but cannot execute until a supported API backend and customer credentials exist. |
...
5. **Native adapter coverage is intentionally smaller than contract breadth.**
   This repo ships 626 native adapter surfaces. 516 overlap the 669 catalog contracts; the remaining catalog contracts are not product-executable until ported.
