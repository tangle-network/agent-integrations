...
| Native adapter backends | 513 |
| Native adapter surfaces shipped | 618 |
| Package-runtime backends | 156 |
| Runtime manifest dependencies for catalog-only connectors | 165 |
| Catalog-only connectors exposable behind runtime | 156 |
...
...and 538 more native adapter surfaces.
...
| Native adapter execution | Done for listed native backends | 618 reviewed native adapter surfaces ship from this package; 513 overlap the 669 catalog contracts. |
| OAuth/API-key setup metadata | Partial | 244 setup specs exist; 222 are executable setup specs and 22 are catalog/setup-only. |
| Direct adapter backlog | Tracked | 156 contracts still need native/direct adapters before they should be product-executable. |
...
| --- | ---: | --- |
| Contracts needing native/direct adapters | 156 | Connector has a Tangle contract but no reviewed direct adapter yet. |
...
1. **Tangle first-class connector contracts are complete.**
   All 669 catalog entries have Tangle-owned contracts. 513 use native adapter backends; 156 are backlog for native ports.
...
5. **Native adapter coverage is intentionally smaller than contract breadth.**
   This repo ships 618 native adapter surfaces. 513 overlap the 669 catalog contracts; the remaining catalog contracts are not product-executable until ported.
...
- It is accurate to say: **the remaining 156 catalog-only contracts are backlog, not runtime-ready product surface.**
