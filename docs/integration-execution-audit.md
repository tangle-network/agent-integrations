...
| Native adapter backends | 495 |
| Native adapter surfaces shipped | 594 |
| Package-runtime backends | 174 |
| Runtime manifest dependencies for catalog-only connectors | 175 |
| Catalog-only connectors exposable behind runtime | 174 |
| Catalog-only actions exposable behind runtime | 797 |
...
...and 514 more native adapter surfaces.
...
| Native adapter execution | Done for listed native backends | 594 reviewed native adapter surfaces ship from this package; 495 overlap the 669 catalog contracts. |
| OAuth/API-key setup metadata | Partial | 231 setup specs exist; 199 are executable setup specs and 32 are catalog/setup-only. |
| Direct adapter backlog | Tracked | 174 contracts still need native/direct adapters before they should be product-executable. |
...
| --- | ---: | --- |
| Contracts needing native/direct adapters | 174 | Connector has a Tangle contract but no reviewed direct adapter yet. |
...
1. **Tangle first-class connector contracts are complete.**
   All 669 catalog entries have Tangle-owned contracts. 495 use native adapter backends; 174 are backlog for native ports.
...
5. **Native adapter coverage is intentionally smaller than contract breadth.**
   This repo ships 594 native adapter surfaces. 495 overlap the 669 catalog contracts; the remaining catalog contracts are not product-executable until ported.
...
- It is accurate to say: **the remaining 174 catalog-only contracts are backlog, not runtime-ready product surface.**
