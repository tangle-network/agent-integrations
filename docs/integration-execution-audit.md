...
| Setup specs | 245 |
| Executable setup specs | 226 |
| Catalog/setup-only specs | 19 |
...
| Native adapter surfaces shipped | 621 |
...
...and 541 more native adapter surfaces.
...
| Native adapter execution | Done for listed native backends | 621 reviewed native adapter surfaces ship from this package; 515 overlap the 669 catalog contracts. |
| OAuth/API-key setup metadata | Partial | 245 setup specs exist; 226 are executable setup specs and 19 are catalog/setup-only. |
...
| Commercial/setup-only provider contracts | 19 | Provider is discoverable with honest setup metadata but cannot execute until a supported API backend and customer credentials exist. |
...
5. **Native adapter coverage is intentionally smaller than contract breadth.**
   This repo ships 621 native adapter surfaces. 515 overlap the 669 catalog contracts; the remaining catalog contracts are not product-executable until ported.
